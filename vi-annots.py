"""Compute plate-well mapping to treatment (compounds) from disparate csvs."""
import numpy as np
import pandas as pd
import itertools
from glob import glob
annots = pd.concat([pd.read_csv(fn)
                    for fn in glob('kirsty-compound-annotation/*Batch*.csv')],
                   axis=0)
plate_codes = pd.read_csv('PlateID_UniqueIdentifier.csv')
plate_mid = pd.read_csv('kirsty_plates.csv')
late_annots = pd.concat([pd.read_csv(fn, sep='\t')
                         for fn in glob('kirsty-compound-annotation/*.tsv')],
                        axis=0)
controls = pd.read_csv('kirsty-compound-annotation/'
                       'Annotation_Kirsty_Controls.csv')

annots['concentration'] = annots['concentration'].astype(str)
annots['summary'] = (annots['libary.id']
                     .str.cat(annots[['sample.no', 'concentration']],
                              sep=' - '))
annots.rename(columns={'libary.id': 'library', 'sample.no': 'compound'},
              inplace=True)
annots['coord'] = annots['plate.id'].str.cat(annots['well'], sep='-')

late_annots.dropna(axis=0, subset=['PURPOSE'], inplace=True)
late_annots['concentration'] = (late_annots['CONC_SUPPLIED']
                                .astype(int).astype(str))
late_annots['summary'] = (late_annots['PURPOSE'].str.cat(
                          late_annots[['QCL_SAMPLE_NUMBER', 'concentration']],
                          sep=' - '))
late_annots.rename(columns={'PURPOSE': 'library',
                            'QCL_SAMPLE_NUMBER': 'compound',
                            'PLATE_ID': 'plate.id'}, inplace=True)
late_annots['coord'] = (late_annots['plate.id'].str.cat(
                        late_annots['TARGET_COORDINATES'], sep='-'))
...
barcode2plateid = dict(zip(plate_mid['plate_barcode'],
                           plate_mid['plate_kind']))
plate_codes.replace({'20170924-KTC-07A rescan': '20170924-KTC-07A'},
                    inplace=True)
desc2barcode = dict(zip(plate_codes['Unique Plate Descriptor'],
                        plate_codes['PlateId/Barcode']))
...
controls2 = (controls[['Well_ID', 'summary', 'library',
                       'compound', 'concentration']]
             .rename(columns={'Well_ID': 'well'}).set_index('well'))

empty_df = pd.DataFrame({'summary': [None], 'library': [None],
                         'compound': [None], 'concentration': [None]})
dfs = []
